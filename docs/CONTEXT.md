...
- Health Atlas: creata e aggiornata `docs/HEALTH.md` come snapshot sintetico. Stato dopo l'audit completo del `2026-05-26`: verdi Atlas, Pratix, FiscalBay e TRAM; in attenzione DocMolder, GLM, SendChimp, SyncBay e Sentinel; nessun progetto bloccato.
...
- Audit completo `2026-05-26`: nessuna PR aperta su Atlas, Pratix, DocMolder, FiscalBay, GLM, SendChimp, SyncBay, TRAM e Sentinel. Inboxes Codex aperte su Pratix `#34`, DocMolder `#149`, FiscalBay `#69`, GLM `#3`, SendChimp `#2`, TRAM `#2` e Sentinel `#4`; Atlas non ha ancora una inbox propria e SyncBay non ha un'issue inbox aperta. Per questo snapshot non viene usato come segnale canonico il checkout locale temporaneamente in lavorazione di SyncBay e TRAM.
- Cleanup branch `2026-05-26`: GLM non ha più la branch remota residua `origin/work-codex-excel-sync`; DocMolder è tornata su `main` pulito, senza branch locale `codex/align-atlas-doc-governance`, con commit docs-only `bf54615` pubblicato. La venv locale DocMolder è stata riparata rigenerando l'entrypoint `coverage`; `bash scripts/ci_test.sh --coverage` passa. Il run remoto `Release Please` su `bf54615` è fallito senza log disponibili; `Release Please` e `VPS Check` sono disabilitati manualmente durante la finestra Actions.
