...
- Ciclo correttivo standard Atlas: creata `docs/STANDARDS.md`; pubblicate uniformazioni sicure su SendChimp `#21`, Pratix `#157`, SyncBay `#30`, Sentinel `#2` e GLM commit `[skip ci]`. GitHub Actions non vanno usate come gate nella finestra fino al `2026-06-01` compreso, ma questo non equivale a freeze completo dello stato remoto: al `2026-05-26` risultano ancora `Release Please` fallita su DocMolder, `CI` fallita su GLM e `PR Title` fallita su SyncBay e Sentinel. Workflow `Codex PR comments`, `Release Please`, `VPS Check` e runtime `Sentinel` restano disabilitati dove previsto. Dependabot resta standard Atlas pieno e non viene declassato dalla sospensione Actions.
- Audit completo `2026-05-26`: nessuna PR aperta su Atlas, Pratix, DocMolder, FiscalBay, GLM, SendChimp, SyncBay, TRAM e Sentinel. Inboxes Codex aperte su Pratix `#34`, DocMolder `#149`, FiscalBay `#69`, GLM `#3`, SendChimp `#2`, TRAM `#2` e Sentinel `#4`; Atlas non ha ancora una inbox propria e SyncBay non ha un'issue inbox aperta. TRAM è stata ricontrollata e ripulita; SyncBay è stata ricontrollata e resta su branch locale feature non pubblicata.
...
- Ricontrollo SyncBay `2026-05-26`: checkout locale pulito su `codex/inventory-import-stock`, con commit unico non pubblicato `398f95f` sopra `origin/main` e nessuna PR o branch remota. Verifiche locali passate: `git diff --check`, `npm run lint`, `npm run prisma:validate`, `npm run typecheck`, `npm run build`, `npm run quality:react-doctor` con score `99 / 100` e solo warning, `npm run release:dry-run` senza file aggiornati. Non pubblicare questa branch durante la sospensione Actions senza decisione esplicita.
