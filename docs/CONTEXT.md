...
- Ciclo correttivo standard Atlas: creata `docs/STANDARDS.md`; pubblicate uniformazioni sicure su SendChimp `#21`, Pratix `#157`, SyncBay `#30`, Sentinel `#2` e GLM commit `[skip ci]`. GitHub Actions non vanno usate come gate nella finestra fino al `2026-06-01` compreso, ma questo non equivale a freeze completo dello stato remoto: al `2026-05-26` restano failure storici o remoti da ricontrollare su DocMolder `Release Please`, GLM `CI`, TRAM `Repo Hygiene`, SyncBay `PR Title` e Sentinel `PR Title`. Workflow `Codex PR comments`, `PR Title`, `Release Please`, `VPS Check` e runtime `Sentinel` restano disabilitati dove previsto. Dependabot resta standard Atlas pieno e non viene declassato dalla sospensione Actions.
