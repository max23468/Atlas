...
| Pratix | Attenzione | PR `#159` mergiata su `main`; `SECURITY.md`, `docs/SECURITY_HARDENING.md` e `docs/ROADMAP.md` aggiornati con hardening P0/P1; Vercel production `READY` con commit `59fbb05`; `/` e `/novita` rispondono `200`; nessuna release SemVer richiesta dal dry-run (`Non versionato`) | P0/P1 bloccanti finché non sono completi: nessuna emissione funzionale nuova finché non chiusi scan/guardrail | Completare checklist Pratix-hardening (segreti, guardrail pre-merge, verifica CI) |
