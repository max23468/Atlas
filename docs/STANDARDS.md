...
| GitHub baseline | PR template, issue template, PR title check o equivalente che accetti Conventional Commit con scope opzionale, policy PR/merge | `OK equivalente`: baseline presente; alcune repo usano `pr-title.yml`, altre controlli equivalenti nella CI/policy locale |
...
| Verifiche | Gate proporzionati al rischio, senza test inventati, con mappa esplicita tra tipo di modifica, corsia e comandi minimi dove serve | `OK pieno`: modello test/verifiche applicato con comandi repo-specifici; run remote storiche sono contesto e non sostituiscono verifiche locali dichiarate |
| React Doctor | Obbligatorio per app React dopo release major/minor o modifiche React trasversali; comando standard `npm run quality:react-doctor` con `npx --yes react-doctor@latest` | `OK pieno`: applicato in Pratix, GLM, SendChimp, SyncBay, TRAM e Sentinel |
