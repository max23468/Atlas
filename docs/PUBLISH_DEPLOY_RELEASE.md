...
| GLM | PR/merge GitHub | Locale dichiarata; tag/GitHub Release sono decisione pending prima di release prodotto reale | Sì, Cloudflare Pages quando richiesto | Guide GLM `versioning-e-release` e `cloudflare-pages`, `docs/DECISIONS_PENDING.md` | Non usare Vercel/Supabase |
| SendChimp | PR/merge GitHub | Locale dichiarata; tag/GitHub Release sono decisione pending prima di release prodotto reale | Vercel production esiste, ma deploy runtime non va attivato senza decisione | `npm run release`, Vercel CLI solo se policy lo richiede, `docs/DECISIONS_PENDING.md` | MVP manuale; niente invii reali o provider nuovi |
| SyncBay | PR/merge GitHub | Locale dichiarata; tag/GitHub Release/App Store production sono decisione pending | Vercel automatico; App Store production non attivo | Guide SyncBay `git-e-pubblicazione`, `versioning-e-release`, `provisioning-runtime`, `docs/DECISIONS_PENDING.md` | Shopify/eBay, no marketplace generico |
