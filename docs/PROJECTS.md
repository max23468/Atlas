...
| TRAM | Allineamento Atlas e fix React Doctor completati e mergiati su `main` | Next.js 16, React 19, TypeScript, npm, Vitest, Python documentale `3.12` locale | GitHub privata `max23468/TRAM`; Quality, Repo Hygiene, PR Title, Codex inbox; PR `#6` e `#7` mergiate | `AGENTS.md`; `README.md`; `docs/INDEX.md`; `docs/ROADMAP.md`; `docs/BACKLOG.md`; `docs/CONTEXT.md`; `docs/TOOLCHAIN.md`; `docs/OPERATIONS.md`; `docs/DECISIONS.md`; `docs/decisions/` | Evidence-first; AI free-first e governata; basename Markdown univoci; non anticipare V2/V3; non inviare documenti sensibili a provider senza policy; nessun deploy/release implicito | Scegliere un debito reale solo con nuova conferma: pilot Fase 7, policy release, provider/data policy o hardening T1/T2/T3 | Policy release/deploy non ancora definita |
| Sentinel | Consolidamento Atlas completato; monitor operativo GitHub Actions per Ortix e San Carlo Sviluppo | Node.js/TypeScript, CLI `sentinel`, config YAML, Vitest; output `data/`, `snapshots/`, `reports/` | `/Users/Matteo/Progetti/Sentinel`; `main`; GitHub privata `max23468/Sentinel`; PR `max23468/Sentinel#1` mergiata; workflow manuale `26369906474` verde; ultimo output commit `4b9d151` | `AGENTS.md`; `README.md`; `docs/INDEX.md`; `docs/ROADMAP.md`; `docs/BACKLOG.md`; `docs/CONTEXT.md`; `docs/TOOLCHAIN.md`; `docs/decisions/`; `package.json`; `sentinel.config.yml`; `.github/workflows/sentinel.yml`; `src/` | Rispettare `robots.txt`; non committare segreti/email; non salvare HTML completo; output applicativi committabili dal workflow | Osservare il prossimo run schedulato e promuovere solo problemi reali da report/log | Ortix ha 33 avvisi 404 ma 0 cambiamenti; Codex inbox lasciata in backlog finché non ci sono review ricorrenti; retention output da valutare solo se cresce |
...
2. pianificare il prossimo ciclo di manutenzione solo quando emerge un nuovo segnale.
