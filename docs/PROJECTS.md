...
| SyncBay | Primo allineamento Atlas completato e release locale `0.6.0` pubblicata su `main`; esiste lavoro locale successivo non pubblicato | Shopify app; eBay sorgente catalogo; Vercel/Supabase; Vercel automatico presente ma no tag/GitHub Release/App Store production | Nessuna PR aperta; nessuna branch remota extra; nessuna inbox Codex aperta; checkout locale pulito su `codex/inventory-import-stock`, commit unico `398f95f` non pubblicato; ultimo run GitHub: `PR Title` fallita il `2026-05-26`; Dependabot presente | `AGENTS.md`; `README.md`; `docs/INDEX.md`; `docs/ROADMAP.md`; `docs/BACKLOG.md`; `docs/CONTEXT.md`; `docs/TOOLCHAIN.md`; `docs/decisions/`; `docs/decisions-pending.md`; `docs/guides/git-e-pubblicazione.md`; `docs/guides/versioning-e-release.md`; `docs/guides/provisioning-runtime.md` | Non trasformare in marketplace generico bidirezionale; App Store solo dopo decisione; niente tag/GitHub Release finché la policy non cambia | Nessuna azione prodotto Atlas; keyset/OAuth, import live o pubblicazione branch solo se richiesti come scope SyncBay | Branch `codex/inventory-import-stock` verificata localmente (`lint`, `prisma:validate`, `typecheck`, `build`, React Doctor `99 / 100`, release dry-run), ma non pubblicata per non attivare Actions |
