...
| GLM | Web app Cloudflare Pages, ricognizione read-only interrotta | JavaScript/npm; Cloudflare Pages | CI + Codex inbox; baseline GitHub da completare | `AGENTS.md`; source of truth docs da creare o verificare; guide Cloudflare/versioning indicate dal piano | Non usare Vercel/Supabase; non modificare allegati gara o Git LFS senza richiesta; deploy solo su target dichiarato | Prima ondata, priorità 1: catalogo docs, roadmap, backlog, contesto, toolchain, ADR e baseline GitHub | Nessuna patch applicata; non ripartire senza nuova conferma esplicita |
| SendChimp | Runtime Next.js iniziale / MVP manuale | Next.js/React, Vercel Functions Frankfurt, Neon Free/Postgres 17 Frankfurt, Neon Auth | GitHub + Vercel previsto/protetto; PR title e inbox già da verificare | `AGENTS.md`; `docs/context.md`; `docs/decisions/`; `docs/decisions-pending.md`; `docs/guides/git-e-pubblicazione.md`; `docs/guides/versioning-e-release.md` | Vincolo free-tier; nessun invio WhatsApp reale; nessuna integrazione produttiva Meta/Mailchimp/Shopify senza piano; nessun Supabase nel primo scaffold | Prima ondata, priorità 4: migrare roadmap/indice/backlog/toolchain senza perdere decisioni runtime | File locali non tracciati con suffisso ` 2`; verificare capienza residua dei piani gratuiti prima di confermare provider |
...
| Sentinel | Da censire, non ancora repo pronta | Sconosciuto; localmente risulta solo cache `.wrangler` | `/Users/Matteo/Documents/Sentinel` non è repo Git; `max23468/Sentinel` non risultava raggiungibile | Nessuna fonte verificata; `AGENTS.md` assente nel checkout locale | Non trattare cache `.wrangler` come contenuto progettuale; non applicare standard repo-per-repo senza baseline | Prima ondata, priorità 5: censire o inizializzare baseline solo su richiesta esplicita | Mancano Git, GitHub, `AGENTS.md`, sorgenti e policy |
...
La prima ondata proposta resta:

1. GLM;
2. TRAM;
3. SyncBay;
4. SendChimp;
5. Sentinel, solo per censimento o baseline.
