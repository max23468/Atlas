...
| SyncBay | Scaffold runtime / MVP Shopify con primo allineamento Atlas completato | Canonica in `docs/` con roadmap, index, backlog, context e toolchain | `npm run release`, `app/lib/version.ts` | Pubblicazione GitHub; Vercel automatico presente ma no release/App Store production decisa | Inbox, template, Dependabot; CI minima; PR `max23468/SyncBay#27` e `#28` mergiate | typecheck, lint, build, smoke UI, db verify; React Doctor 100/100 su PR #27 | Scegliere prossimo debito reale: release locale 0.6.0 o keyset/OAuth eBay |
...
3. SyncBay: completato primo allineamento Atlas e chiusa inbox; prossimo solo debito reale, per esempio release locale 0.6.0 o keyset/OAuth eBay.
...
| SyncBay | completata | Roadmap migrata in `docs/ROADMAP.md`; creati `docs/INDEX.md`, `docs/BACKLOG.md`, `docs/TOOLCHAIN.md`; `docs/CONTEXT.md` canonico; `ROADMAP.md` e `docs/README.md` restano rinvii | PR `max23468/SyncBay#27` e `#28` mergiate; Codex inbox senza thread actionable; Docker Node allineato a engine | PR #27: `npm run typecheck`, `npm run lint`, `npm run build`, `npm run quality:react-doctor` 100/100, `npm run release:dry-run`, `git diff --check`; PR #28: `git diff --check`, `npm run release:dry-run`, Vercel pass; Docker build locale non eseguito perché daemon non avviato | Shopify app; no marketplace generico; no release/App Store production finché non deciso; release locale 0.6.0 resta prossimo step possibile |
