...
- `rilascia`: crea o chiude una versione, con changelog e, per ogni release prodotto reale, tag Git e GitHub Release `vX.Y.Z`;
...
7. rilasciare solo se la repo ha release policy o se il comando utente lo richiede; ogni release prodotto reale crea tag Git e GitHub Release `vX.Y.Z`;
...
| --- | --- | --- | --- |
| Pratix | PR/merge GitHub verso main | `npm run release` secondo policy; release prodotto reale con tag Git e GitHub Release `vX.Y.Z` | Vercel production quando previsto |
| DocMolder | PR/merge GitHub | policy release manuale repo-specifica; release prodotto reale con tag Git e GitHub Release `vX.Y.Z` | VPS/runbook quando previsto |
| FiscalBay | PR/merge GitHub o flusso repo | `scripts/release_now.sh` quando previsto; release prodotto reale con tag Git e GitHub Release `vX.Y.Z` | `scripts/deploy_now.sh`/VPS quando previsto |
| GLM | PR/merge GitHub | `npm run release` quando previsto; release prodotto reale con tag Git e GitHub Release `vX.Y.Z` | Cloudflare Pages solo su richiesta/target dichiarato |
| SendChimp | PR/merge GitHub | release locale solo per pre-release; release prodotto reale con tag Git e GitHub Release `vX.Y.Z` | Vercel solo quando previsto e protetto da Neon Auth; nessun invio automatico |
| SyncBay | PR/merge GitHub | `npm run release` locale solo per pre-release; release prodotto reale con tag Git e GitHub Release `vX.Y.Z` | Vercel production pilota; Shopify App Store production non attiva |
| TRAM | PR/merge GitHub | SemVer `0.x` secondo ADR `0003`; release prodotto reale con tag Git e GitHub Release `vX.Y.Z` | non applicabile finché manca target deploy approvato |
| Atlas | push/PR verso repository pubblica GitHub | non applicabile | non applicabile |
| Sentinel | GitHub pubblica `max23468/Sentinel`, commit su `main` | tag Git e GitHub Release `vX.Y.Z` obbligatori per release tool/dashboard | GitHub Actions schedulata/dispatch per runtime; dashboard Vercel/Blob separata |
